...
void allocation_list_insert(allocation_list_ptr* head_holder, const void * address)
{
    ...
    allocation_list_ptr new_head = malloc(sizeof(allocation_list_type));
...
void allocation_list_remove(allocation_list_ptr* head_holder, const void * address)
{
    ...
    for (allocation_list_ptr* current = head_holder; *current != NULL; ) {
        if (address != (*current)->allocated_address) {
            current = &((*current)->next);
            continue;
        }

        allocation_list_ptr to_delete = *current;
        *current = (*current)->next;
        free(to_delete);
        count++;
    }
...
void allocation_list_to_string(allocation_list_cptr head, char * buffer, int max_size)
{
    char local_buffer[20];

    for (allocation_list_cptr current = head; current != NULL; current = current->next) {
