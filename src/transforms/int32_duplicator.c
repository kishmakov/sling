...
static const char* int32_duplicator_input_scheme(transform_cref transform)
{
    (void) transform;
...
static const char* int32_duplicator_output_scheme(transform_cref transform)
{
    (void) transform;
    static char* scheme = NULL;
...
static const char* int32_duplicator_profile(transform_cref transform)
{
    (void) transform;
...
static transform_hld int32_duplicator_construct(void_mv seed)
{
    *seed = NULL;

    transform_hld result = malloc(sizeof(transform_type));
    result->internal_data = seed;
...
{
    void * dummy = NULL;
    return int32_duplicator_construct(&dummy);
}
